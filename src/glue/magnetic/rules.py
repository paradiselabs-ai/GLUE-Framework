...
from .field import AttractionStrength, MagneticResource, ResourceState
...
class AttractionPolicy(Enum):
    ...
    STRENGTH_BASED = auto()  # Allow based on strength comparison
...
class AttractionRule:
    ...
    min_strength: AttractionStrength = AttractionStrength.WEAK
    description: str = ""
    enabled: bool = True
    ...
    def validate(
        ...
        if self.policy == AttractionPolicy.STRENGTH_BASED:
            return (
                source.strength.value >= self.min_strength.value and
                target.strength.value >= self.min_strength.value
            )
